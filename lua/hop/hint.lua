        ...
        for _, hint in pairs(hints.hints) do
            if vim.fn.strdisplaywidth(hint.hint) == 1 then
                vim.api.nvim_buf_set_extmark(
                    ...
                    hint.line,
                    hint.col - 1,
                    {virt_text = {{hint.hint, "HopNextKey"}}, virt_text_pos = "overlay"}
                )
            else
                ...
                vim.api.nvim_buf_set_extmark(
                    ...
                    {
                        virt_text = {
                            {hint.hint:sub(1, snd_idx), "HopNextKey1"},
                            {hint.hint:sub(snd_idx + 1), "HopNextKey2"}
                        },
